...
final class TmuxParserTests {
    ...
    private static void testParserAvoidsStreamPipelines(TestMain tests) throws Exception {
        ...
        tests.check(!source.contains(".isBlank("), "tmux parser avoids unicode blank scans");
        ...
        tests.check(!source.contains("parseInt(") && !source.contains("parseLong("), "tmux parser parses numeric fields from row ranges");
        tests.check(!source.contains("\"1\".equals(required(row"), "tmux parser parses flags from row ranges");
        ...
        tests.check(!source.contains("session(ArrayList<MutableSession> sessions, String name)"), "tmux parser avoids uncached parent session scans");
